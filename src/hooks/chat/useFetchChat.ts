...
import { ChatUser, Message } from "@/src/lib/types/chat";
...
export default function useFetchChat({
    ...
    const fetchMessages = async () => {
      ...
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}help/chat/orderChat/?limit=10&page=1&orderId=${chatId}`,
          {
            headers: {
  ...
  const loadMoreMessages = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}help/chat/all?limit=20&page=${
          currentPage + 1
        }&orderId=${chatId}`,
        {
          headers: {
      ...
      if (olderMessages.length === 0) {
        ...
        setMessages((prevMessages) => {
          ...
          if (messagesToAdd.length > 0) {
            ...
            console.log(
              `📚 API: Added ${messagesToAdd.length} messages, total now: ${newMessages.length}`
            );
            return newMessages;
          }
