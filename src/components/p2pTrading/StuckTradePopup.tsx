...
interface PopupProps {
  ...
  onClose: () => void;
  trade: Trade;
  setShowResolvePopup: React.Dispatch<React.SetStateAction<boolean>>;
  setFavor: React.Dispatch<React.SetStateAction<string>>;
  status: string;
}
      ...
      <div className="relative max-h-full w-full max-w-md md:max-w-lg">
        ...
        <div className="relative rounded-2xl bg-white shadow-lg">
          ...
          <div className="flex flex-col items-center p-6 font-[satoshi]">
            <h3 className="text-xl font-semibold text-gray-900 mb-5 mt-10">
              Trade Details
            </h3>
            {/* Currency Icon */}
            <div
          ...
          <div className="flex gap-4 w-full justify-between px-10 md:px-16 pb-8">
            {status.toLowerCase() !== "resolved" && (
              <>
                <button
                  className=" flex justify-center items-center gap-2 px-5 py-2 text-xs border border-primary rounded-xl text-primary bg-white hover:bg-primary-50"
                  onClick={() => {
                    setFavor("Buyer");
                ...
                <button
                  className="flex justify-center items-center gap-2 px-5 py-2 rounded-xl font-semibold text-white bg-primary hover:bg-blue-900"
                  onClick={() => {
                    setFavor("Seller");
                    setShowResolvePopup(true);
                  }}
